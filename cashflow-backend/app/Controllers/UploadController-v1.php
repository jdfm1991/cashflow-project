...
class UploadController
{
    ...
    public function uploadBankStatement(): void
    {
        ...
        if (!$parser) {
            unlink($tempPath);
            Response::error('Formato de banco no soportado. Por favor, seleccione un banco válido.', 400);
            return;
        }
        ...
        if (!$parser->validateFormat($tempPath)) {
            unlink($tempPath);
            Response::error(
                'El archivo no tiene el formato esperado para ' . $parser->getConfig()['name'] . '. ' .
                    'Por favor, verifique que está usando el formato correcto.',
                400
            );
            return;
        }
